env                   = "dev"
aws-region            = "eu-north-1"
vpc-cidr-block        = "10.16.0.0/16"
vpc-name              = "dev-eks-cluster-vpc"
igw-name              = "dev-eks-cluster-igw"
pub-subnet-count      = 2
pub-cidr-block        = ["10.16.0.0/20", "10.16.16.0/20"]
pub-availability-zone = ["eu-north-1a", "eu-north-1b"]
pub-sub-name          = "dev-eks-cluster-subnet-public"
pri-subnet-count      = 2
pri-cidr-block        = ["10.16.128.0/20", "10.16.144.0/20"]
pri-availability-zone = ["eu-north-1a", "eu-north-1b"]
pri-sub-name          = "dev-eks-cluster-subnet-private"
public-rt-name        = "dev-eks-cluster-public-route-table"
private-rt-name       = "dev-eks-cluster-private-route-table"
eip-name              = "dev-eks-cluster-elasticip-ngw"
ngw-name              = "dev-eks-cluster-ngw"
eks-sg                = "dev-eks-cluster-eks-sg"
aws_region = "us-east-1"
# EKS
is-eks-cluster-enabled     = true
cluster-version            = "1.31"
cluster-name               = "dev-eks-cluster"
endpoint-private-access    = true
endpoint-public-access     = false
ondemand_instance_types    = ["t3.medium"]
spot_instance_types        = ["c5.large", "c5.xlarge", "m5.large", "c5.large", "m5.large", "t3.large", "t3.xlarge", "t3.medium"]
desired_capacity_on_demand = "1"
min_capacity_on_demand     = "1"
max_capacity_on_demand     = "5"
desired_capacity_spot      = "1"
min_capacity_spot          = "1"
max_capacity_spot          = "10"
addons = [
  {
    name    = "vpc-cni",
    version = "v1.19.2-eksbuild.1"
  },
  {
    name    = "coredns"
    version = "v1.11.4-eksbuild.1"
  },
  {
    name    = "kube-proxy"
    version = "v1.31.3-eksbuild.2"
  },
  {
    name    = "aws-ebs-csi-driver"
    version = "v1.38.1-eksbuild.1"
  }
  # Add more addons as needed
]
